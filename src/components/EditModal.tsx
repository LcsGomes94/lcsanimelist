...
export default function EditModal() {
    ...
    return (
        <div className={`flex justify-center items-center fixed inset-0 bg-black/50 z-50 transition-opacity duration-200 ${transition === 100 ? 'opacity-100' : 'opacity-0'}`}
            onClick={() => {
                setTransition(0)
                setTimeout(() => {
                    handleCloseEditModal()
            ...
            <div className={`relative flex flex-col items-center gap-5 md:gap-7 w-full md:w-[460px] max-w-[356px] md:max-w-[460px] bg-white dark:bg-gray-900 rounded-xl p-5 md:p-7 pt-4 md:pt-6`}
                onClick={e => {
                    e.stopPropagation()
                }}>
                <button
                ...
                <div className={`flex justify-between w-full`}>
                    <button className={`flex gap-1.5 md:gap-2 items-center text-sm md:text-base ${removeFavorite ? 'cursor-not-allowed opacity-50' : ''}`} disabled={removeFavorite} onClick={() => {
                        selectedStage !== 'finished' && setSelectedStage('finished')
                    }}>
                        <div className={`flex items-center justify-center rounded-full border md:border-2 h-3.5 md:h-4 w-3.5 md:w-4 ${selectedStage === 'finished' ? 'border-cyan-400' : 'border-gray-400 dark:border-gray-500'}`} >
                    ...
                    <button className={`flex gap-1.5 md:gap-2 items-center text-sm md:text-base ${removeFavorite ? 'cursor-not-allowed opacity-50' : ''}`} disabled={removeFavorite} onClick={() => {
                        ...
                        <div className={`flex items-center justify-center rounded-full border md:border-2 h-3.5 md:h-4 w-3.5 md:w-4 ${selectedStage === 'dropped' ? 'border-cyan-400' : 'border-gray-400 dark:border-gray-500'}`}>
                ...
                <div className={`w-full rounded-xl bg-gray-100 dark:bg-gray-700 overflow-hidden ${removeFavorite ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    <textarea value={personalNote} placeholder='Enter text' disabled={removeFavorite}
                        className={`h-24 md:h-32 w-full bg-inherit text-xs md:text-sm md:leading-[1.125rem] py-1 md:py-2 px-2.5 md:px-4 resize-none textarea-scroll-light dark:textarea-scroll-dark dark:placeholder:opacity-50 ${removeFavorite ? 'cursor-not-allowed' : ''}`}
                        onChange={e => {
                ...
                <div className={`flex justify-center items-center gap-7 md:gap-9`}>
                    ...
                    }} className={`rounded-full md:text-xl w-44 md:w-56 h-8 md:h-10 text-white dark:text-gray-900 ${removeFavorite ? 'bg-red-500 hover:bg-red-600' : 'bg-cyan-500 hover:bg-cyan-400'}`}>
                        Confirm
                    </button>
