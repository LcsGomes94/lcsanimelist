...
export default function UserMenu() {
    ...
    return (
        <div className={`flex justify-end gap-5 lg:gap-6 flex-1 pr-1 md:pr-1.5 transition-opacity duration-700 ${opacity === 100 && status !== 'loading' ? 'opacity-100' : 'opacity-0'}`}>
            <button onClick={handleToggleTheme}
                className={`flex justify-center items-center h-8 md:h-10 lg:h-11 w-8 md:w-10 lg:w-11 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700`}>
                {theme === 'light' ?
            ...
            <button onClick={() => {
                ...
                className={`flex justify-center items-center md:hidden pb-[1px] hover:bg-gray-100 dark:hover:bg-gray-700 h-8 w-8 rounded-full disabled:opacity-50 disabled:cursor-not-allowed`}>
                <SearchIcon className={`h-[22px] w-[22px]`} />
            </button>
            <button onClick={() => handleOpenUserModal()}
                className={`flex md:hidden pl-1.5`}>
